<?php
	...
	include('pages/examples/mails/class.smtp.php');
	include('pages/examples/mails/class.phpmailer.php');
	
	$sql=mysql_query("select * from deliv_detail where enq_id='".$_GET['smd']."'");
		$sql=mysql_query("select * from invoice_details where Enq_id='".$_GET['did']."'");
	while($row=mysql_fetch_assoc($sql))
	{
		...
		$contents .='Dear <strong style="font-size:16px">'.$row['cust_name'].'</strong><br>
					...
					'.$row['model1'].' in '.$row1['color'].' color with ENGINE NUMBER '.$row['engine_no'].' and CHASSIS NUMBER '.$row['chassis_no'].'
					<br>
